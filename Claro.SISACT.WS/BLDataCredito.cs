...
{
    public class BLDataCreditoDNI
    {
        BEUsuarioSession objSession;
        GeneradorLog _objLog = null;

        //PROY-24740
        public BEDataCreditoOUT ConsultarDatosDataCredito(BEDataCreditoIN objDataCreditoIN, BEUsuarioSession objUsuario, ref BEItemMensaje objMensaje)
        {
            ...
            try
            {
                ...
                objDataCreditoOUT = objDataCreditoNegocio.ConsultarDCRepositorioPersona(objDataCreditoIN, tipoSEC, ref objMensaje);
                if (objDataCreditoOUT != null & objMensaje.exito)
                {
                    string param = "Consulta Tabla masiva Exitosa. (";
                    param += objDataCreditoOUT.NUMEROOPERACION + ";" + objDataCreditoIN.istrTipoDocumento + ";" + objDataCreditoIN.istrNumeroDocumento + ";";
                    param += objDataCreditoIN.istrPuntoVenta + ";" + tipoActivacion + ";" + tipoCliente_Venta + ";" + objDataCreditoOUT.ACCION + ";";
                    param += objDataCreditoOUT.RESPUESTA + ";" + objDataCreditoOUT.APELLIDO_PATERNO + ";";
                    param += objDataCreditoOUT.APELLIDO_MATERNO + ";" + objDataCreditoOUT.NOMBRES + ";" + tipoSEC + ";" + objDataCreditoIN.istrEstadoCivil + ";";
                    param += objDataCreditoOUT.RAZONES + ";" + objDataCreditoOUT.ANALISIS + ";" + objDataCreditoOUT.SCORE_RANKIN_OPERATIVO + ";";
                    param += objDataCreditoOUT.NUMERO_ENTIDADES_RANKIN_OPERATIVO + ";" + objDataCreditoOUT.PUNTAJE + ";" + objDataCreditoOUT.LIMITECREDITODATACREDITO + ";";
                    param += objDataCreditoOUT.LIMITECREDITOBASEEXTERNA + ";" + objDataCreditoOUT.LIMITECREDITOCLARO + ";" + objDataCreditoOUT.FECHANACIMIENTO + ";";
                    param += objDataCreditoOUT.CODIGOBURO + ")";

                    Auditoria(ConfigurationManager.AppSettings["DcAuditGuardarConsultaTMasivo"].ToString(), param);
                }
                else
                {
                    // Consulta WS Buro Crediticio
                    WS.BWDataCredito obj = new WS.BWDataCredito();
                    objDataCreditoOUT = obj.ConsultaBuroCrediticio(objDataCreditoIN, ref objMensaje);
                      ...
                      objDataCreditoOUT.RESPUESTA == ConfigurationManager.AppSettings["strConstanteTipo7"].ToString()))
                {
                    ...
                    {
                        if (objDataCreditoOUT.RESPUESTA == obj.Codigo)
                        {
                            strMensaje = obj.Descripcion;
                            break;
                        }
                    }
                ...
                if (blnConsultaDC)
                {
                    ...
                    objINOUT.IODCC_CODIGOBURO = Funciones.CheckInt(objDataCreditoOUT.BUROCONSULTADO); //ADD PROY-20054-IDEA-23849
