...
{
    public static class RestServiceValidarCampana
    {
        private static WebHeaderCollection GetHeaders(Hashtable table)
        {
            GeneradorLog _objLog = null;
            _objLog = new GeneradorLog("Metodo GetHeaders", null, null, "log_RestServiceValidarCampana");
            ...
            string wsIp = Funciones.CheckStr(ConfigurationManager.AppSettings["ValidacionCampanaWsip"]);
            string ipServidor = Funciones.CheckStr(ConfigurationManager.AppSettings["ValidacionCampanaWsip"]);
            string usrAplicacion = Funciones.CheckStr(ConfigurationManager.AppSettings["constUsuarioAplicacion"]);
            ...
            string usuarioAplicacionEncriptado = ConfigurationManager.AppSettings["User_BeneficiosService"];
            string claveEncriptada = ConfigurationManager.AppSettings["Pass_BeneficiosService"];
        ...
        {
            GeneradorLog objLog = new GeneradorLog(null, "Inicio metodo  PostInvoque", null, "RestServiceValidarCampana");
            ...
            objLog.CrearArchivolog("IDEA-142010 Cadena data  -->" + data.ToString(), "", null);
            ...
            using (Stream stream = ws.GetResponseStream())
            {
                ...
                objLog.CrearArchivolog("IDEA142010 RestServiceValidarCampana() ResponseString  Return JSON: -->" + responseString.ToString(), "", null);
